...
export default function OwnerPropertyForm({ property }: OwnerPropertyFormProps) {
  ...
  const [form, setForm] = useState({
    ...
    base_price: Number(property.base_price) || 5000,
    weekend_price: Number(property.weekend_price) || 7000,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await updateProperty(property.id, formattedData);
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <ArrowLeft className="w-3.5 h-3.5" /> Back to My Farmhouses
        </Link>
      </div>
      ...
      <div className="glass-panel p-6 md:p-8 rounded-3xl border border-stone-200/40 dark:border-slate-800/40 shadow-2xl bg-white/70 dark:bg-slate-900/60 backdrop-blur-lg">
        <div className="mb-6">
          ...
          <p className="text-xs text-stone-400 mt-1">
            Modify rates, specs, locations, and media resources for your listing.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-stone-500 uppercase mb-1.5">Weekend Price (Fri-Sun ₹)</label>
              <input
                type="number"
                required
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-stone-50 dark:bg-slate-955 border border-stone-200/60 dark:border-slate-800/60 rounded-xl text-sm font-semibold focus:outline-none focus:border-green-800 focus:ring-1 focus:ring-green-800 transition-all"
              />
            </div>
          ...
          <div className="flex gap-3 pt-4 border-t border-stone-200/40 dark:border-slate-800/40">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-2.5 bg-green-850 hover:bg-green-800 text-white rounded-xl text-xs font-bold shadow-md transition-all flex items-center justify-center gap-1.5 disabled:opacity-40"
            >
